...
@require_GET
def cpi_page(request, username=None):
    if username is None:
    ...
    return render(request, 'user/cpi.html', {
        'userdata': rp.get_udata_from_player(player, username),
        'detail': cpi.details(player) if player else None,
        ...
        'min_charts': cpi.MIN_CHARTS,
        'source_url': cpi.SITE_URL,
    })
