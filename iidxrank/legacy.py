...
def redirect_user(request, username, rest):
    ...
    같은 화면을 준다.
    """
    # 사이트 자신의 경로는 넘기지 않는다. 지금 없는 주소라도 404 로 둔다 —
    # 영구 리다이렉트를 브라우저에 캐시시켜 두면 나중에 그 주소로 페이지를
    # 만들어도 이미 방문한 사람은 계속 옛 자리로 간다. 자세한 사정은
    # iidxrank/reserved.py 참조.
    if reserved.is_site_path(username):
        raise Http404()
